...
export async function loadCityMap(cityId, {fetchImpl=fetch,signal}={}) {
  ...
  if(cityId==='astana'){
    const extras=await Promise.all(Object.entries(ASTANA_MAP_FILES).filter(([key])=>!['seed','roads','buildings','landscape','greenery','buildingManifest'].includes(key)).map(async ([key,path])=>{
