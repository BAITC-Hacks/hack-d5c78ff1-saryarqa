...
const url = process.env.AKIM_SCENE_TEST_URL || 'http://127.0.0.1:3012/scene/dev-fixture.html';
...
try {
  ...
  const afterBlur = await page.evaluate(() => {
    ...
    return window.akimSceneDev.scene.getDiagnostics().mayor.position;
  });
  await page.waitForTimeout(150); await page.keyboard.up('d');
  assert.deepEqual((await diagnostics()).mayor.position, afterBlur, 'blur clears held movement');
